...
with pkgs; {
  ...
  preview = pkgs.writeShellScriptBin "pv" ''
    ...
    preview_image() {
        ...
        if [[ "$( file -Lb --mime-type "$file")" =~ ^image ]]; then
            ...
            ${_ kitty} icat --transfer-mode file --stdin no --place "''${w}x''${h}@''${x}x''${y}" "$file" < /dev/null > /dev/tty
            exit 1
        fi
    ...
    preview_text() {
        ...
        if command -v bat > /dev/null 2>&1; then
            ...
            ${_ bat} --color=always --style=numbers --pager=never "$file" "$@"
        else
            cat "$file"
        fi
    }
    ...
    case "$(printf "%s\n" "$(readlink -f "$1")" | awk '{print tolower($0)}')" in
        ...
        *.avi|*.mp4|*.wmv|*.dat|*.3gp|*.ogv|*.mkv|*.mpg|*.mpeg|*.vob|*.fl[icv]|*.m2v|*.mov|*.webm|*.ts|*.mts|*.m4v|*.r[am]|*.qt|*.divx)
            [ ! -f "''${CACHE}.jpg" ] && ${_ ffmpegthumbnailer} -i "$1" -o "''${CACHE}.jpg" -s 0 -q 5
            preview_image "''${CACHE}.jpg" "$2" "$3" "$4" "$5"
            ;;
  ...
  set previewer ${_ preview}
  set cleaner ${_ cleaner}
